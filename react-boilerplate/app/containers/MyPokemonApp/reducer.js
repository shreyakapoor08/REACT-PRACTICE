/*
 *
 * MyPokemonApp reducer
 *
 */

import { fromJS } from 'immutable';
import { DEFAULT_ACTION,GET_POKEMON_LIST, ON_CLICK_REQUEST, DISPLAY_FORM, ADD_NEW_POKEMONS} from './constants';
...
function myPokemonAppReducer(state = {}, action) {
  let pokemonData = initialState.pokemons;
  ...
  switch (action.type) {
    ...
    case ADD_NEW_POKEMONS: 
        pokemonData.unshift(action.data);
        console.log('pokemonData',pokemonData); 
        return Object.assign({},state,{
       "addNewPokemon": pokemonData })     

    case ON_CLICK_REQUEST:
      pokemonData[action.index].Caught = true; //state[action.index].completed = true;
      console.log("pokemonData[action.index].Caught",pokemonData[action.index].Caught)
    ...
    // return [
    //   ...pokemonData.slice(0, action.index),
    ...
    //     "pokemons": pokemonData
    //   }),
    //   ...pokemonData.slice(action.index + 1)
    // ];
...
export default myPokemonAppReducer;
